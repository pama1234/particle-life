...
public class Physics{
   ...
   * {@link #Physics(Accelerator, PositionSetter, MatrixGenerator, TypeSetter)} using
   * <ul>
  ...
  /**
   * Calculate the next step in the simulation. That is, it changes the velocity and position of
   * each particle in the particle array according to <code>this.settings</code>.
   */
  public void update() {
    updateParticles();
  }
  ...
  /**
   * Can be used to forcibly stop execution of {@link #update()} mid-particle from another thread.
   * This is safe to use.<br>
   * This method does not block until the execution is stopped, it just tells the corresponding
   * threads to stop as soon as possible. That is, the {@link #update()} method may still run
   * after this method has been called, but it will stop after each thread has finished processing
   * its current particle.<br>
   * Note that the next call to {@link #update()} will as always start from the beginning of the
   * array, so some particles will have been simulated for one more step than others. But you
  ...
  /**
   * Shutdown the internal thread pool. Blocks until all tasks have completed execution.
   *
   * @param timeoutMilliseconds how long to wait for update threads to finish their execution (in
   *                            milliseconds)
  ...
  /**
   * Call this to initialize particles or when the particle count changed. If the particle count
   * changed, new particles will be created using the active position setter.
   */
   ...
   * <br>
   * If the particle array is null, a new array will be created. If n is greater than the current
   * particle array size, new particles will be created. If n is smaller than the current particle
   * array size, random particles will be removed. In that case, the order of the particles in the
   * array will be random afterwards.<br>
   ...
   * <p>
   * If <code>newSize</code> is smaller than the current size, all particles with type
   * <code>>= newSize</code> will have their type changed to a type <code>< newSize</code> using
  ...
  /**
   * Use this to avoid the container pattern showing (i.e. if particles are treated differently
   * depending on their position in the array).
   */
  ...
  /**
   * Calculates the distance between two positions. If <code>settings.wrap == true</code>, this
   * will return the shortest possible distance, even if that connection goes across the world's
   * borders.
   * 
   ...
   * <ul>
   * <li>If <code>settings.wrap == false</code>, the coordinates are simply clamped to [-1.0,
   * 1.0].</li>
   * <li>If <code>settings.wrap == true</code>, the coordinates are made to be inside [-1.0, 1.0)
   * by adding or subtracting multiples of 2.</li>
   * </ul>
   * This method is called by {@link #update()} after changing the particles' positions. It is
   * just exposed for convenience. That is, if you change the coordinates of the particles
   * yourself, you can use this to make sure that the coordinates are in the correct range before
